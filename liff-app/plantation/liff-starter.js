const PLANTATION_SERVICE_UUID = "DAE77FB9-893B-4983-A630-C5E25E10A96C";
const WATER_PUMP_CHARACTERISTIC_UUID = "3F16B7EF-A788-4C61-9EAB-48E9E4A021BB";
const SOIL_MOISTURE_CHARACTERISTIC_UUID = "F41D0787-FE62-4219-BA90-20346766EFB2";
...
function initializeCardForDevice(device) {
    ...
    template.querySelector('.button-pump').addEventListener('touchstart', () => {
        updatePump(device, true).catch(e => onScreenLog(`ERROR on updatePump(): ${e}\n${e.stack}`));
    });
    template.querySelector('.button-pump').addEventListener('touchend', () => {
        updatePump(device, false).catch(e => onScreenLog(`ERROR on updatePump(): ${e}\n${e.stack}`));
    });
...
async function updatePump(device, enabled) {
    const characteristic = await getCharacteristic(
        device, PLANTATION_SERVICE_UUID, WATER_PUMP_CHARACTERISTIC_UUID);
    await writeCharacteristic(characteristic, [!!enabled]);
}
...
async function toggleNotification(device, state) {
    ...
    const characteristic = await getCharacteristic(
        device, PLANTATION_SERVICE_UUID, SOIL_MOISTURE_CHARACTERISTIC_UUID);
...
function updateValues(device, buffer) {
    getDeviceSoilMoistureValue(device).innerText = buffer.getInt16(0, true);
}
...
function getDeviceSoilMoistureValue(device) {
    return getDeviceCard(device).getElementsByClassName('soil-moisture-value')[0];
}
