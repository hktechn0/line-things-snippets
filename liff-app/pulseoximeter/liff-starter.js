...
const PLX_FEATURES_CHARACTERISTIC_UUID = "00002A60-0000-1000-8000-00805F9B34FB";
const PLX_CONTINUOUS_MEASUREMENT_UUID = "00002A5F-0000-1000-8000-00805F9B34FB";
...
function updateValues(device, buffer) {
    const sfloatSpo2 = buffer.getInt16(1, true);
    const sfloatPulseRate = buffer.getInt16(3, true);

    getDevicePulseRateValue(device).innerText = readSFLOAT(sfloatPulseRate).toFixed(2) + " bpm";
    getDeviceSpO2(device).innerText = readSFLOAT(sfloatSpo2).toFixed(2) + " %";
}

function readSFLOAT(dataInt16) {
    const exponential = dataInt16 >> 12;
    let mantissa = (dataInt16 & 0x0FFF);
...
function getDevicePulseRateValue(device) {
    return getDeviceCard(device).getElementsByClassName('pulserate-value')[0];
}

function getDeviceSpO2(device) {
    return getDeviceCard(device).getElementsByClassName('spo2-value')[0];
}
