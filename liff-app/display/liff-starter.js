...
function initializeCardForDevice(device) {
    ...
    template.querySelector('.button-add-text').addEventListener('click', () => {
        ...
        const textElement = textElements.children.slice(-1)[0].cloneNode(true);
...
async function updateText(device) {
    ...
    await writeCharacteristic(textCharacteristic, textPayload);
    await writeCharacteristic(colorCharacteristic, colorPayload);
}
