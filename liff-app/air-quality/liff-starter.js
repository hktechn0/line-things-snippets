...
const AIR_QUALITY_MONITOR_SERVICE_UUID = "288BDFE4-7F85-47A6-9908-3B8F29CE69E5"
const CO2_CHARACTERISTIC_UUID = "9F3F3F1D-3763-4D1F-8031-58C728D7794B"
const TVOC_CHARACTERISTIC_UUID = "00CF24AA-BC94-437E-ABD4-241D26F72779"
...
async function refreshValues(device) {
    ...
    const co2Characteristic = await getCharacteristic(
        device, AIR_QUALITY_MONITOR_SERVICE_UUID, CO2_CHARACTERISTIC_UUID);
    const tvocCharacteristic = await getCharacteristic(
        device, AIR_QUALITY_MONITOR_SERVICE_UUID, TVOC_CHARACTERISTIC_UUID);
    ...
    const co2Buffer = await readCharacteristic(co2Characteristic).catch(e => {
        return null;
    });
    const tvocBuffer = await readCharacteristic(tvocCharacteristic).catch(e => {
        return null;
    });

    if (pressureBuffer !== null) {
        const pressureValue = pressureBuffer.getUint32(0, true) / 10.0 / 100.0;
        getDevicePressureField(device).innerText = `${pressureValue.toFixed(0)} hPa`;
    }
    if (temperatureBuffer !== null) {
        const temperatureValue = temperatureBuffer.getInt16(0, true) / 100.0;
        getDeviceTemperatureField(device).innerText = `${temperatureValue.toFixed(1)} ℃`;
    }
    if (humidityBuffer !== null) {
        const humidityValue = humidityBuffer.getUint16(0, true) / 100.0;
        getDeviceHumidityField(device).innerText = `${humidityValue.toFixed(1)} %`;
    }
    if (co2Buffer !== null) {
        const co2Value = co2Buffer.getUint16(0, true);
        getDeviceCo2Field(device).innerText = `${co2Value} ppm`;
    }
    if (tvocBuffer !== null) {
        const tvocValue = tvocBuffer.getUint16(0, true);
        getDeviceTvocField(device).innerText = `${tvocValue} ppb`;
    }
}
...
function getDeviceCo2Field(device) {
    return getDeviceCard(device).getElementsByClassName('co2-value')[0];
}

function getDeviceTvocField(device) {
    return getDeviceCard(device).getElementsByClassName('tvoc-value')[0];
}
